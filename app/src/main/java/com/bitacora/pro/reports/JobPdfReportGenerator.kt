...
object JobPdfReportGenerator {
    ...
    fun generateReport(
        ...
        return try {
            ...
            // Draw report notes section if available
            if (job.reportNotes.isNotBlank()) {
                yPosition = drawReportNotesSection(currentCanvas, job, yPosition)
                yPosition += 20
            }
    ...
    private fun drawReportNotesSection(canvas: Canvas, job: JobFile, startY: Int): Int {
        val headerPaint = Paint().apply {
            color = Color.parseColor("#00897B")
            textSize = 14f
            isFakeBoldText = true
        ...
        // Section header
        canvas.drawText("Notas del Reporte", MARGIN.toFloat(), yPos.toFloat(), headerPaint)
        yPos += 18

        // Split notes into lines and draw them
        val lines = job.reportNotes.split("\n")
        lines.forEach { line ->
            if (line.isNotBlank()) {
                // Simple text wrapping for long lines
                val wrappedLines = wrapText(line, CONTENT_WIDTH, notePaint)
                wrappedLines.forEach { wrappedLine ->
                    canvas.drawText("• $wrappedLine", (MARGIN + 10).toFloat(), yPos.toFloat(), notePaint)
                    yPos += 14
                }
            }
        }
    ...
    private fun drawFooter(canvas: Canvas, yPos: Int) {
        ...
        canvas.drawText("Generated by Bitacora Pro v0.9.0 - $timestamp", MARGIN.toFloat(), yPos.toFloat(), footerPaint)
    }
